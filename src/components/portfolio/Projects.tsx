...
const projectList = [
  {
    ...
    features: ["Authentication", "One-to-one messaging", "End-to-end encryption", "Responsive UI"],
    accent: "from-violet-500 to-fuchsia-500",
    mock: (
      <div className="space-y-2">
        <div className="flex justify-start"><div className="rounded-2xl rounded-bl-sm bg-white/10 px-3 py-1.5 text-[11px]">Hey! 👋</div></div>
        <div className="flex justify-end"><div className="rounded-2xl rounded-br-sm bg-gradient-to-r from-violet-500 to-cyan-400 px-3 py-1.5 text-[11px] text-background">Yo, encrypted?</div></div>
        <div className="flex justify-start"><div className="rounded-2xl rounded-bl-sm bg-white/10 px-3 py-1.5 text-[11px] flex items-center gap-1"><Lock className="h-3 w-3" /> end-to-end</div></div>
      </div>
    ),
  },
  {
    ...
    features: ["Product listing", "Product detail", "Cart & Checkout", "Payments", "Admin panel"],
    accent: "from-cyan-500 to-emerald-500",
    mock: (
      <div className="grid grid-cols-3 gap-2">
        {[0,1,2,3,4,5].map((i) => (
          <div key={i} className="aspect-square rounded-lg bg-gradient-to-br from-white/10 to-white/5 border border-white/5" />
        ))}
      </div>
    ),
  },
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          {projectList.map((project, index) => (
            ...
            >
              <div className={`absolute -top-32 -right-32 h-64 w-64 rounded-full bg-gradient-to-br ${project.accent} opacity-10 blur-3xl transition-opacity group-hover:opacity-20`} />
              ...
              <div className="relative flex items-start justify-between gap-4">
                <div>
                  <div className="flex items-center gap-2">
                    <div className={`grid h-8 w-8 place-items-center rounded-lg bg-gradient-to-br ${project.accent}`}>
                  ...
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    {project.tech.map((tech) => (
                      <span key={tech} className="rounded-md border border-white/10 bg-white/5 px-2 py-0.5 text-[10px] text-muted-foreground">
                        {tech}
                      </span>
                    ))}
                  </div>
