  ...
  return (
    <section id="experience" className="relative px-4 py-24 md:py-32">
      <div className="mx-auto max-w-4xl">
        <SectionHeader eyebrow="Experience" title={<>Where I've <span className="gradient-text">shipped</span></>} />
          ...
          <div className="space-y-12">
            {workHistory.map((job, index) => (
              <FadeIn key={job.company} delay={index * 0.1}>
                ...
                <div className={`relative md:grid md:grid-cols-2 md:gap-10 ${index % 2 === 1 ? "md:[direction:rtl]" : ""}`}>
                  {/* dot */}
                  ...
                  <div className={`pl-10 md:pl-0 ${index % 2 === 1 ? "md:[direction:ltr] md:col-start-2 md:pl-10" : "md:pr-10 md:text-right"}`}>
                    <div className="glass gradient-border rounded-2xl p-6 transition-all hover:-translate-y-1 hover:shadow-[0_20px_60px_-20px_rgba(167,139,250,0.4)]">
                      ...
                      <div className="text-sm gradient-text font-medium">{job.company}</div>
                      <ul className={`mt-4 space-y-2 text-sm text-muted-foreground ${index % 2 === 1 ? "md:[direction:ltr]" : ""}`}>
                        {job.bullets.map((b) => (
